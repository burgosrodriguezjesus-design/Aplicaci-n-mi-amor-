...
| `db-url.mts` | Las direcciones de Supabase y Neon se ajustan para ir por su *pooler* con Prisma (sin eso fallan 70 de cada 96 consultas con «prepared statement does not exist»), y para crear las tablas se usa la dirección que sí lo admite. Comprueba también que la copia de la lógica del paso de construcción dice exactamente lo mismo. |
